...
export function remoteValidator($parse) {
  return {
    restrict: 'A',
    require: ['ngModel', '?^busyIndicator'],
    link: function(scope, elm, attr, ctrls) {
...
export function busyIndicator($compile) {
  return {
    scope: true,
    transclude: true,
...
export function ajaxButton($compile, $animate) {
  return {
    transclude: true,
    restrict: 'E',
    scope: {
...
export function owlCarousel($compile, $timeout) {
  var owl = null;
  return {
    scope: {
      options: '=',
